...
public:
  ...
  void set_flags(bool is_final_flag, bool is_volatile_flag, bool is_tsan_ignore_flag) {
    int new_flags = (is_final_flag << is_final_shift) |
                    static_cast<int>(is_volatile_flag) |
                    (is_tsan_ignore_flag << is_tsan_ignore_shift);
    _flags = checked_cast<u1>(new_flags);
    assert(is_final() == is_final_flag, "Must be");
    assert(is_volatile() == is_volatile_flag, "Must be");
    assert(is_tsan_ignore() == is_tsan_ignore_flag, "Must be");
  }
