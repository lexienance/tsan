...
void TemplateTable::tsan_observe_get_or_put(const Address &field,
  ...
  if (tos == atos) {
    int32_t acquire_release_mask = 1 << ResolvedFieldEntry::is_volatile_shift |
      1 << ResolvedFieldEntry::is_tsan_ignore_shift;
    // acquire_release_mask (0x5) can not be encoded into 'tst', but it can be
    ...
    __ tbz(flags, ResolvedFieldEntry::is_volatile_shift, notAcquireRelease);
  }
  ...
  if (ThreadSanitizerJavaMemory) {
    ...
    __ tbnz(flags, ResolvedFieldEntry::is_final_shift, done);

    // Don't report races on tsan ignored fields.
    __ tbnz(flags, ResolvedFieldEntry::is_tsan_ignore_shift, done);
